...
- **系统托盘** — 关闭窗口后最小化到托盘，单击恢复，支持退出菜单
- **窗口置顶** — 标题栏 📌 按钮切换窗口始终在最上层
- **窗口记忆** — 自动保存窗口位置和大小，重启恢复
...
**体验优化** — 修复启动 bug，新增系统托盘、窗口置顶、窗口记忆，代码重构。

| 提交 | 说明 |
|------|------|
| — | 修复启动时不加载今日待办的 Bug |
| — | 系统托盘常驻：关闭窗口最小化到托盘，单击恢复，右键菜单退出 |
| — | 窗口位置记忆：自动保存窗口位置和大小，重启自动恢复 |
| — | 窗口置顶：标题栏 📌 按钮切换始终在最上层 |
| — | 内联样式迁移至 CSS 文件，组件代码更简洁 |
| — | 添加/切换/删除操作改用本地状态更新，消除多余数据库查询 |
| — | 提取共享 `getToday()` 工具函数，消除重复代码 |
| — | 清理 DateQuery 未使用的 ConfirmDialog 导入 |
